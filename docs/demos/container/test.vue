<template>
  <QuestionPopover :categories="questions">
    <div>hello</div>
  </QuestionPopover>
</template>

<script setup lang="ts">
import { QuestionPopover } from '@opentiny/tiny-robot'
import { IconLike, IconDislike } from '@opentiny/tiny-robot-svgs'

const questions = [
  {
    id: 'basic',
    label: '推荐',
    icon: IconLike,
    questions: [
      { id: 'b1', text: '什么是弹性云服务器?' },
      { id: 'b2', text: '如何登录到Windows云服务器?' },
      { id: 'b3', text: '弹性公网IP为什么ping不通?' },
      { id: 'b4', text: '云服务器安全组如何配置?' },
      { id: 'b5', text: '如何查看云服务器密码?' },
    ],
  },
  {
    id: 'purchase',
    label: '购买咨询',
    icon: IconDislike,
    questions: [
      { id: 'p1', text: '如何购买弹性云服务器?' },
      { id: 'p2', text: '无法登录弹性云服务器怎么办?' },
      { id: 'p3', text: '云服务器价格怎么计算?' },
      { id: 'p4', text: '如何查看账单详情?' },
      { id: 'p5', text: '如何续费云服务器?' },
    ],
  },
  {
    id: 'usage',
    label: '使用咨询',
    icon: IconLike,
    questions: [
      { id: 'u1', text: '云服务器使用限制与须知' },
      { id: 'u2', text: '使用RDP文件连接Windows实例' },
      { id: 'u3', text: '多用户登录（Windows2016）' },
      { id: 'u4', text: '如何重置云服务器密码?' },
      { id: 'u5', text: '云服务器如何安装软件?' },
    ],
  },
  { id: '4', label: '推荐', icon: IconLike, questions: [] },
  { id: '5', label: '购买咨询', icon: IconLike, questions: [] },
  { id: '6', label: '使用咨询', icon: IconLike, questions: [] },
  { id: '7', label: '购买咨询', icon: IconLike, questions: [] },
  { id: '8', label: '使用咨询', icon: IconLike, questions: [] },
  { id: '9', label: '购买咨询', icon: IconLike, questions: [] },
  { id: '10', label: '使用咨询', icon: IconLike, questions: [] },
]
</script>
