...
export class AliyunSpeechHandler implements SpeechHandler {
  ...
  private async processWithAliyunAPI(audioBlob: Blob): Promise<void> {
    ...
    try {
      const baseUrl = 'https://agent.opentiny.design/api/asr/recognize'
      ...
      if (result.success && result.data?.result) {
        ...
        throw new Error(result.message || result.data?.message || '识别失败')
      }
    } catch (error) {
      this.callbacks.onError(error instanceof Error ? error : new Error('语音识别失败'))
    }
  }
