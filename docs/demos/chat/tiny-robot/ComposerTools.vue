...
function fillTemplateMenuItem(item: unknown): void {
  const template = (item as { template?: TemplateSuggestionItem }).template
  if (template) fillTemplate(template)
}
  ...
  <div class="chat-add-tools">
    ...
    <TrSuggestionPills class="chat-add-tools__pills">
      ...
      <TrDropdownMenu
        ...
        trigger="click"
        @item-click="fillTemplateMenuItem"
      >
        <template #trigger
