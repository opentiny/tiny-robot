...
- 将原始事实保存为本地 fixture。
- 读取所有通过权限与 bot 检查的新 Issue 评论，结合当前状态判断是否包含 Review 意见或自然语言控制请求。
- 调用 CLI 获取确定性判断。
- 根据结果执行调研、写作、润色、人工沟通和流程编排。

CLI 负责：

- 标记首个非空位置以独立 `/ai` 前缀开头的显式请求，但不解释其自然语言意图。
- 校验固定写作计划批准命令。
- 过滤 bot 和未授权评论。
- 校验写作计划固定批准命令并生成批准快照。
- 校验文章、项目配置、路径和素材契约。
...
CLI 标准化作者权限、标记显式 `/ai` 请求并校验固定批准命令。Agent 对所有授权、非 bot 的新评论做 triage：明确 Review 意见无需 `/ai` 前缀，普通讨论无需处理，显式控制请求映射到封闭 intent。CLI 不判断 Review 或控制意图，只校验固定批准命令、当前状态和 mutation。写作计划批准仍以固定命令和批准快照为准。
