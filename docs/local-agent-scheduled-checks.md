...
不要把本文整篇复制到普通 Agent 对话里。本文是说明书，真正给 Agent 的巡检任务在两个独立文件中：
...
- Codex app automation：在对话中明确要求“创建定时任务”，指定工作目录、频率、运行方式和要使用的巡检任务文件。Issue 巡检和 PR 巡检应创建成两个任务。
- 调度平台或 Cron：在产品的定时任务入口中创建两个任务，分别让 Agent 读取 Issue 巡检和 PR 巡检任务文件。
...
每次被定时唤醒时，Agent 必须遵守以下规则。完整任务规则以 `docs/prompts/local-issue-watch.md` 和 `docs/prompts/local-pr-watch.md` 为准。
