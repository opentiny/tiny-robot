...
- 输入时实时显示联想弹窗。
- 支持鼠标悬停预览和键盘 `↑`/`↓` 键切换预览。
- 按 `Tab` 或 `Enter` 键确认选中项，内容将回填至输入框。
- 输入框内（目前主要在单行模式下）会显示用户已输入内容和半透明的补全提示文本。
- 当存在联想项时，输入框右侧会显示一个虚线边框的 `Tab` 提示器。
- 联想弹窗每条内容包含图标（当前为默认图标）和文本。
...
- **输入框补全**: 在单行模式下，用户输入部分正常显示，联想到的补全部分以半透明灰色文本展示。
- **联想弹窗**: 
    - 内容结构：左侧图标，右侧文本。
    - 交互：鼠标悬停项高亮，键盘导航同步高亮，并更新输入框的补全提示。
    - 关闭时机：点击外部区域、按 `Esc` 键、选中联想项后。
- **键盘交互**:
    - `↑`/`↓`: 在联想弹窗中导航。
    - `Tab`/`Enter`: 确认当前高亮的联想项。
    - `Esc`: 关闭联想弹窗。

> **注意**: 输入框内的补全文本特性在 `mode="multiple"` (多行模式) 下因实现复杂性目前未精确支持，但联想弹窗和选择功能仍然可用。
