...
| `layout.rightAside`                         | 未配置时不显示；配置对象默认宽度 `320` |
...
宿主按需维护：`data.conversation`、`data.bubble.messages`、`data.sender` 和 `data.request`，以及创建、切换、重命名、删除和历史菜单事件。未提供的数据字段使用组件默认值。
...
| `layout-header`                                    | 替换默认 Header 内容，并接管侧栏按钮行为      |
| `layout-left-aside`                                | 替换左侧栏展开面板                            |
| `layout-left-aside-brand` / `layout-left-aside-actions` | 定制左侧栏品牌或操作区                  |
| `layout-left-aside-content` / `layout-left-aside-footer` | 定制左侧栏内容或底部区域                |
| `layout-left-aside-rail`                           | 定制 Dock 折叠态内容                          |
| `layout-left-aside-history-item-prefix`            | 定制默认历史项前缀                            |
| `layout-right-aside`                               | 替换右侧栏容器                                |
| `layout-right-aside-title` / `layout-right-aside-panel` | 定制右侧栏标题或面板正文                 |
| `layout-main`                                      | 替换消息和空状态主内容                        |
| `layout-empty-state`                               | 仅在没有可见消息时替换默认空状态              |
| `composer-before`                                  | 输入区前置内容                                |
| `layout-footer`                                    | 替换输入区内部的默认 Sender 内容              |
| `sender-header` / `sender-footer` / `sender-footer-right` | 定制 Sender 的头部、底部或右侧底部内容 |
...
`layout-main` 会保留 ChatUI 的滚动宿主和滚动控制；`layout-footer` 会保留外层 Footer 和输入区容器，只替换内部默认 Sender。自定义输入或布局内容需要自行承担原有事件、键盘、ARIA、焦点和状态责任。

`layout-main` 优先于 `layout-empty-state`。没有 `layout-main` 且没有可见消息时，`layout-empty-state` 才会渲染；没有这两个自定义主区插槽时，才使用默认 Welcome 和 Prompts。默认 Composer 独立遵循 `sender` 和 `layout.composer.welcome` 配置；自定义空状态激活时，只有显式调用 `renderComposer()` 才会渲染它。

`layout-empty-state` 的 Slot Props 为：`messages`、`request`、`conversation`、`isEmpty: true` 和 `renderComposer`。自定义空状态不会自动渲染默认 Composer，也不受默认空状态居中规则约束；需要复用默认 Composer 时，在自定义内容中调用一次 `renderComposer()`。
...
右栏 `width` 作为桌面端初始宽度；桌面端使用 `mode: 'dock'` 且设置 `resizable: true` 后才可拖拽调整宽度，拖拽结果会由 ChatUI 内部保留。`minWidth` 和 `maxWidth` 分别限制拖拽边界；无效数值会被忽略，`maxWidth` 小于 `minWidth` 时按 `minWidth` 处理。`mode: 'drawer'` 和移动端右栏均不启用 resize；右栏 resize 不暴露公共事件。
...
| `update:input-value`                                 | `string`                        | `TrChatUI` 受控输入值更新           |
...
| `bubble-state-change` / `bubble-event`               | 对应气泡事件 payload             | 气泡状态或交互事件                  |
...
| `ChatConversationView` / `ChatBubbleView` / `ChatSenderView` / `ChatRequestView` | UI 会话、消息、输入和请求状态 |
| `ChatModelView` / `ChatModelOptionView` / `ChatMcpView` | 模型和 MCP 展示数据 |
| `ChatMcpServerView` / `ChatMcpToolView` / `ChatMcpToolMap` | MCP Server 和工具展示数据 |
...
| `ChatRightAsidePanelContext`                                                                              | 右侧栏标题和面板的公共插槽上下文 |
| `ChatHeaderSlotProps` / `ChatLeftAsideSlotProps` / `ChatLeftAsideContentSlotProps` | Header 和左侧栏插槽参数 |
| `ChatHistoryItemPrefixSlotProps` / `ChatRightAsideTitleSlotProps` / `ChatRightAsidePanelSlotProps` | 历史项和右侧栏插槽参数 |
| `ChatMainSlotProps` / `ChatEmptyStateSlotProps` / `ChatSenderSlotProps` | 主区、空状态和输入区插槽参数 |
| `ChatBubbleSlotProps` / `ChatBubbleContentFooterSlotProps` | 气泡插槽参数 |
