...
- 校验评论作者权限。
- 用确定性规则标记评论首个非空位置的独立 `/ai` 前缀，排除引用和文本内提及。
- 让 LLM 读取所有通过权限和 bot 检查的新 Issue 评论，判断是否包含需要处理的 Review 意见或显式控制请求；普通讨论不执行动作。
- 用确定性规则校验固定写作计划批准命令。
...
LLM 不能依赖固定命令 parser 判断 Review 或控制意图。所有通过权限和 bot 检查的新评论都进入 triage：明确 Review 意见无需 `/ai` 前缀，显式控制请求根据评论原文和 Issue 当前事实提出 `status`、`pause`、`resume`、`retry` 或写作计划修改 intent，普通讨论标记为无需处理。原文不能直接决定工具调用，mutation job 必须用确定性状态机复核。批准意图不能交给 LLM 判断，只接受固定 `/ai 批准写作计划`。
