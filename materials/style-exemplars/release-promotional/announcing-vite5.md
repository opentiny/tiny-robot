...
> 标题：Vite 5.0 is out!
> 作者：Vite Team
> 出处 URL：https://vite.dev/blog/announcing-vite5
> 采集日期：2026-06-25
...
If you're new to Vite, we suggest reading first the [Getting Started](https://vite.dev/guide/) and [Features](https://vite.dev/guide/features) guides.
...
Vite 5 also introduces [server.warmup](https://vite.dev/guide/performance#warm-up-frequently-used-files), a new feature to improve startup time. It lets you define a list of modules that should be pre-transformed as soon as the server starts. When using [`--open` or `server.open`](https://vite.dev/config/server-options#server-open), Vite will also automatically warm up the entry point of your app or the provided URL to open.

## Main Changes

-   [Vite is now powered by Rollup 4](https://vite.dev/guide/migration#rollup-4)
-   [The CJS Node API has been deprecated](https://vite.dev/guide/migration#deprecate-cjs-node-api)
-   [Rework `define` and `import.meta.env.*` replacement strategy](https://vite.dev/guide/migration#rework-define-and-import-meta-env-replacement-strategy)
-   [SSR externalized modules value now matches production](https://vite.dev/guide/migration#ssr-externalized-modules-value-now-matches-production)
-   [`worker.plugins` is now a function](https://vite.dev/guide/migration#worker-plugins-is-now-a-function)
-   [Allow path containing `.` to fallback to index.html](https://vite.dev/guide/migration#allow-path-containing-to-fallback-to-index-html)
...
-   [Manifest files are now generated in `.vite` directory by default](https://vite.dev/guide/migration#manifest-files-are-now-generated-in-vite-directory-by-default)
-   [CLI shortcuts require an additional `Enter` press](https://vite.dev/guide/migration#cli-shortcuts-require-an-additional-enter-press)
...
-   [Remove `--https` flag and `https: true`](https://vite.dev/guide/migration#remove-https-flag-and-https-true)
-   [Remove `resolvePackageEntry` and `resolvePackageData` APIs](https://vite.dev/guide/migration#remove-resolvepackageentry-and-resolvepackagedata-apis)
-   [Removes previously deprecated APIs](https://vite.dev/guide/migration#removed-deprecated-apis)
-   [Read more about advanced changes affecting plugin and tool authors](https://vite.dev/guide/migration#advanced)
...
For most projects, the update to Vite 5 should be straight forward. But we advise reviewing the [detailed Migration Guide](https://vite.dev/guide/migration) before upgrading.
...
Vite 5 is the result of long hours of work by our community of contributors, downstream maintainers, plugins authors, and the [Vite Team](https://vite.dev/team). A big shout out to [Bjorn Lu](https://twitter.com/bluwyoo) for leading the release process for this major.
