...
`articles/publications.json` 记录新流程文章在各平台的成功发布事实。文件不管理发布任务，不记录待发布、失败或下架状态；缺少文章条目或平台记录表示尚未确认发布。
...
文章发布到平台后，在 `articles` 中按文章目录加入条目。`article_id` 由 `article_file` 派生：去掉 `articles/` 前缀和 `/article.md` 后缀。
...
- 文章条目字段建议按 `article_file`、`topic_issue`、`source_pr`、`publications` 排列；省略不存在的可选字段。
