#!/usr/bin/env node
/**
 * 将 marked.md 中的 __CSDN_IMG_N__ 占位符替换为 CSDN CDN URL。
 *
 * 用法：
 *   node replace.mjs --marked marked.md --replacements replacements.json --out fixed.md
 *
 * replacements.json 形如: { "0": "https://img-blog.csdnimg.cn/...", "1": "https://..." }
...
function main() {
  ...
  const placeholderRe = /__CSDN_IMG_(\d+)__/g;
  const found = new Set();
  ...
  const still = [...marked.matchAll(/__CSDN_IMG_(\d+)__/g)].map((m) => m[1]);
  ...
  if (unresolvedFinal.length) {
    console.log(
      JSON.stringify({
        ...
        errors: unresolvedFinal.map((k) => `missing or invalid url for __CSDN_IMG_${k}__`),
      }),
    );
    process.exit(1);
  }
