#!/usr/bin/env node
/**
 * 将 marked.md 中的 __JUEJIN_IMG_N__ 占位符替换为掘金 CDN URL。
 *
 * 用法：
 *   node replace.mjs --marked marked.md --replacements replacements.json --out fixed.md
 *
 * replacements.json 形如: { "0": "https://p3-juejin.byteimg.com/...", "1": "https://..." }
...
function main() {
  ...
  const placeholderRe = /__JUEJIN_IMG_(\d+)__/g;
  const found = new Set();
  ...
  // 仍残留的占位符
  const still = [...marked.matchAll(/__JUEJIN_IMG_(\d+)__/g)].map((m) => m[1]);
  ...
  if (unresolvedFinal.length) {
    console.log(
      JSON.stringify({
        ...
        errors: unresolvedFinal.map((k) => `missing or invalid url for __JUEJIN_IMG_${k}__`),
      }),
    );
    process.exit(1);
  }
