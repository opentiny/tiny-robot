 ...
 *
 * 推荐后续步骤：把 --out 产出交给 upload-imagex.mjs（ImageX 五步，编辑器页注入）。
 * execute_javascript_hint 仅为过时 FormData 占位，实战勿再用。
 *
 * 用法：
 *   node prepare-upload.mjs --markers markers.json --index 0 [--out upload-0.json]
 *
 * stdout: JSON { ok, index, filename, mimeType, base64, local_path, byte_length, ... }
 */
 ...
 */
/**
 * 过时 FormData 提示（掘金实际为 ImageX；保留字段兼容旧调用方）。
 * @param {string} filename
 * @param {string} mimeType
...
function main() {
  ...
  const full = {
    ...
    note: 'Preferred next step: node upload-imagex.mjs --upload-json <this-out> --out run.json --poll-out poll.json then page-agent-tool -f on the editor tab.',
  };
